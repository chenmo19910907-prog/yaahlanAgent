...
def build_direct_cases() -> list[TestCase]:
    ...
    return [
        TestCase(
            name=fname,
            args=["--payload-file", str(moa_template(fname))],
...
def build_cli_cases() -> list[TestCase]:
    return [
        TestCase(
            name="room_exp_add",
            args=[
                "--payload-file", _tpl("房间经验值-backdoor.json"),
        ...
        TestCase(
            ...
            args=[
                "--payload-file", _tpl("VIP-增加经验值.json"),
        ...
        TestCase(
            ...
            args=[
                "--payload-file", _tpl("家族-增加声望值.json"),
                "--family-id", "101435",
        ...
        TestCase(
            ...
            args=[
                "--payload-file", _tpl("家族-增加基金贡献值.json"),
                "--family-id", "101435",
        ...
        TestCase(
            ...
            args=[
                "--payload-file", _tpl("钻石-查询余额.json"),
        ...
        TestCase(
            ...
            args=[
                "--payload-file", _tpl("魅力-查询等级.json"),
        ...
        TestCase(
            ...
            args=[
                "--payload-file", _tpl("财富-查询等级.json"),
        ...
        TestCase(
            ...
            args=[
                "--payload-file", _tpl("用户-按手机号查userId.json"),
        ...
        TestCase(
            ...
            args=[
                "--payload-file", _tpl("查询用户登录天数.json"),
                "--expr", "100465989",
            ],
            category="cli-variant",
        ),
...
def run_case(case: TestCase) -> tuple[bool, str, int | None]:
    cmd = [sys.executable, str(moa_execute_path()), *case.args]
    try:
        proc = subprocess.run(
            cmd,
            cwd=str(repo_root()),
...
def main() -> int:
    ...
    print(f"模板目录: {moa_templates_dir()}\n")

    for case in all_cases:
        ok, msg, ec = run_case(case)
