...
- [8) 房间管理（room/external/room-admin-handle）](#moa-cat-8)
  - [房间-下发小时榜全服通知](#room_hourly_top1_broadcast)
- [9) 房间经验值（voga-mts-room-backdoor）](#moa-cat-9)
  - [房间-增加机器人](#room_add_bots)
  - [房间等级-升级到目标等级](#room_level_upgrade)
  - [房间经验值-增加](#room_exp_add)
  - [房间经验值-查询当前等级经验](#room_query_current)
- [10) 支付测试（pay-middle/test）](#moa-cat-10)
  - [送礼限制-查询测试账号](#pay_test_query_all)
- [11) 用户后门（voga-mts-user-backdoor）](#moa-cat-11)
  - [用户-注销账号](#user_cancel_real)
- [12) 用户大区（yaahlan/components/callback/user-area）](#moa-cat-12)
  - [用户-修改大区](#user_area_change)
- [13) 用户活跃（yaahlan/user/internal/area-moa）](#moa-cat-13)
  - [用户-查询登录天数](#user_active_days_query)
- [14) 用户登录（yaahlan/mdp-user-login）](#moa-cat-14)
  - [用户-按手机号查 userId](#user_login_query_by_phone)
- [15) 用户资料（voga-mts-user-profile-stage）](#moa-cat-15)
  - [用户-查看 app 语言](#user_app_language_query)
- [16) 背包礼物（voga-base-service-middle-gift-stage）](#moa-cat-16)
  - [背包礼物-下发](#package_gift_add)
- [17) 贵族（voga-mts-user-wealth-charm-level-stage）](#moa-cat-17)
  - [贵族-升级到目标等级](#noble_level_upgrade)
  - [贵族-增加月消费值](#noble_exp_add)
- [18) 钻石（voga-base-service-middle-pay-stage）](#moa-cat-18)
...
## 8) 房间管理（room/external/room-admin-handle）
...
## 9) 房间经验值（voga-mts-room-backdoor）
...
## 11) 用户后门（voga-mts-user-backdoor）
...
## 12) 用户大区（yaahlan/components/callback/user-area）
...
## 13) 用户活跃（yaahlan/user/internal/area-moa）
...
## 14) 用户登录（yaahlan/mdp-user-login）
...
## 15) 用户资料（voga-mts-user-profile-stage）
...
## 16) 背包礼物（voga-base-service-middle-gift-stage）
...
## 17) 贵族（voga-mts-user-wealth-charm-level-stage）
...
## 18) 钻石（voga-base-service-middle-pay-stage）
