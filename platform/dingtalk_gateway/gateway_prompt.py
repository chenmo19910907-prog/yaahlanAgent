...
from project.loader import gateway_agent_name, temporary_testcase_dir, web_login_phrase  # noqa: E402
...
_GATEWAY_RULES = None
_READONLY_GATEWAY_RULES = None
_READONLY_WITH_MOA_REGISTRY_RULES = None


def _web_login_rule_line() -> str:
    phrase = web_login_phrase()
    return (
        f"20. **网页验证码**：用户发送「{phrase}」申请网页版验证码时，**验证码只能私聊发送**；"
        "在群里回复时**禁止**明文展示验证码，仅提示「已私发」。该口令由网关快捷路由处理，Agent **不要**代为生成或回复验证码。"
    )
...
def _gateway_rules() -> str:
    global _GATEWAY_RULES
    if _GATEWAY_RULES is not None:
        return _GATEWAY_RULES
    agent_name = gateway_agent_name()
    tmp_dir = _temporary_testcase_hint()
    _GATEWAY_RULES = f"""\
你是 {agent_name}，在钉钉群无人值守场景下运行。
...
def _readonly_gateway_rules() -> str:
    global _READONLY_GATEWAY_RULES
    if _READONLY_GATEWAY_RULES is not None:
        return _READONLY_GATEWAY_RULES
    agent_name = gateway_agent_name()
    tmp_dir = _temporary_testcase_hint()
    _READONLY_GATEWAY_RULES = f"""\
你是 {agent_name}（**只读模式**），在钉钉群无人值守场景下运行。
...
def _readonly_with_moa_registry_rules() -> str:
    global _READONLY_WITH_MOA_REGISTRY_RULES
    if _READONLY_WITH_MOA_REGISTRY_RULES is not None:
        return _READONLY_WITH_MOA_REGISTRY_RULES
    agent_name = gateway_agent_name()
    tmp_dir = _temporary_testcase_hint()
    _READONLY_WITH_MOA_REGISTRY_RULES = f"""\
你是 {agent_name}（**只读模式 · 可 MOA 入库**），在钉钉群无人值守场景下运行。
