#!/usr/bin/env python3
"""为定制礼物后台列表中的全部用户下发 VIP5 体验（默认 1 小时）。"""
...
from moa.params import set_vip_try_dispatch_params  # noqa: E402
...
def dispatch_vip5_try(
    ...
    *,
    ...
    if not isinstance(payload, dict):
        raise ValueError("VIP 体验卡模板必须是 JSON object")
    set_vip_try_dispatch_params(payload, user_id, level, duration_seconds)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="定制礼物后台用户批量下发 VIP5 体验")
    parser.add_argument("--per-page", type=int, default=500, help="getListConfig 每页条数")
    parser.add_argument("--duration-seconds", type=int, default=3600, help="体验时长（秒），默认 3600=1 小时")
    parser.add_argument("--vip-level", type=int, default=5, help="体验 VIP 等级，默认 5")
    parser.add_argument("--dry-run", action="store_true", help="只列出 userId，不调用 MOA")
    parser.add_argument(
        "--user-ids",
        help="逗号分隔 userId；指定时跳过定制礼物列表查询",
    )
    ...
    template = MOA_DIR / "templates" / "VIP-下发体验卡.json"
