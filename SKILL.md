...
1. **处理流程**：
   ...
   **若为版本需求**，额外执行：
   ...
   - **【覆盖目标】** 按 `version_testcase_generation_rules.md` 要求覆盖：功能验收、边界值、多角色、屏幕适配（有前端变更时）、多语言（英/阿/土/俄，所有需求均需覆盖）、**图片资源格式（SVGA/JPG/PNG/WEBP/GIF，涉及图片时）**
   ...
   **通用步骤（所有需求）**：
   ...
   - 生成每个模块的用例时，除了根据产品原型生成全部的测试用例，还需要参考对应规则文件（版本需求用 `version_testcase_generation_rules.md`，活动需求用 `testcase_generation_rules.md`）里的通用规则进行用例补充（含屏幕适配、多语言、**图片格式 SVGA/JPG/PNG/WEBP/GIF**、动态效果档位/极速模式等）
   - 用例格式根据使用场景分两种：
