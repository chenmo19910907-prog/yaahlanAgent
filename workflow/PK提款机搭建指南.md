...
**本文档讲什么**：PK 提款机**测试搭建步骤**——从录 MOA 到 Sheet2 一键验收，按步执行即可。

**搭建流程（五步）**：

1. **录 MOA**——抓包跨房 PK / 送礼 / 结束，写入 MOA 模板  
2. **对话试跑**——两号开 PK，C1～C9 边跑边改  
3. **落验收脚本**——`pk_atm_dingtalk_sheet2_run.py` + 工作流 `pk-atm-test`  
4. **Sheet2 写表**——每轮自动新建钉钉 Sheet「测试结果-*」  
5. **一键重测**——改参数直跑，stdout 回 `workbookUrl` + `sheetName`

**术语对照**：PRD = 产品需求文档 · MOA = 测试环境后台接口 · 抓包 = 记录 App 的网络请求 · 造数 = 模拟一场完整 PK · Sheet2 = 钉钉表里的「测试结果」页
...
每步验收通过再进下一步；**禁止**跳过试跑直接要自动化。

```mermaid
flowchart LR
  P1[① 录 MOA<br>抓包→模板] --> P2[② 对话试跑<br>C1～C9]
  P2 --> P3[③ 落脚本<br>sheet2+工作流]
  P3 --> P4[④ Sheet2 验收<br>页面+返钻+排名]
  P4 --> P5[⑤ 一键重测<br>改参写钉钉表]
```

| 步 | 做什么 | 验收 |
|----|--------|------|
| ① 录 MOA | 跨房 PK / 送礼 / 结束写入模板 | MOA 命令可独立执行 |
| ② 对话试跑 | 两号匹配 → 送礼 → 结束 | C1～C9 跑通 |
| ③ 落脚本 | `pk_atm_dingtalk_sheet2_run.py` | 本地跑通一轮 |
| ④ Sheet2 | 弹窗 / 排名 / 吸底 / 实发 | 钉钉表「测试结果-*」有记录 |
| ⑤ 一键重测 | 改 `--target-combined-pk` 等参数 | stdout 含表格链接 |

---

## 2. 阶段 ①：先把规则搞清楚（不用写代码）

目标：开测之前，AI 能答——什么 PK 算有效、返多少钻、边界情况怎么处理。
...
## 3. 阶段 ②：录接口，做成 MOA（一次录一个）

目标：开 PK、送礼、结束 PK 等动作，都有**可重复执行的命令**，以后不用手点 App。
...
目标：在**同一条对话**里，用大白话把一场 PK 从头跑到尾；缺什么就补一句。

### 4.1 九轮对话都干了什么（C1～C9）

| 轮次 | 你大概说了什么 | 白话：这步解决啥 |
|------|----------------|------------------|
| C1 | 两号随机匹配开 PK，20 秒后 20 个号送礼，再结束 | **最小流程**：能跑通一场有效 PK |
| C2 | 然后两房间麦上各加 5 个机器人 | 麦上得有人，礼物才送得出去 |
| C3 | 送给麦上的人，1～1000 钻随机；送完等 20 秒再结束 | 修正送给谁、送多少、什么时候结束 |
| C4 | 结束时记录各房 PK、谁赢、每人贡献多少 | 出一份**能看懂的报告** |
| C5 | 把以上能力结合到 PK 提款机测试里 | 从临时试跑 → **正式测试方案** |
| C6 | 结束前记余额，结束后比对钻石变化是否等于应返钻 | **核对返钻对不对**（个人未达门槛应为 0） |
| C7 | PK 还没结束时，检查赛况页双方分数、对战信息 | **进行中页面**数据对不对 |
| C8 | PK 结束后查返钻弹窗、提款排行榜、底部「本周已提款」 | **结束后页面**和弹窗对不对 |
| C9 | 再跑一轮 XX 万总 PK，每人送礼 XX～XX 钻 | 改参数重跑，结果写入钉钉表 |
...
### 4.5 一键脚本里验什么（C6 → C7 → C8）

`pk-atm-test` 一条命令跑完的**顺序**（对话里用 C6/C7/C8 三句逐步补全的）：
...
| 阶段 | 验什么（白话） | 怎么查 | 状态 |
|------|----------------|--------|------|
| **C6** | 返钻数量对不对 | 查钱包余额前后差 | 已有；**个人 PK 不足 1 万应返 0** |
| **C7** | PK 进行中页面分数对不对 | 调赛况接口 | 已有 |
| **C8** | 弹窗、排行榜、底部「本周已提款」 | 调排名接口 + 读弹窗 | 已验 |
| **C9** | 改总 PK / 送礼金额重跑 | Sheet2 脚本 | **Web Agent 常用** |
...
线上跨房 PK / 公屏活动卡片等场景使用以下 **4 个 MENA 账号**（2026-08-11 Admin 线上 queryUserDetail 核实）：

| # | userId | roomId |
|---|--------|--------|
| 1 | 107427060 | 53334066 |
| 2 | 107347491 | 38458582 |
| 3 | 107343529 | 82548523 |
| 4 | 107488084 | 55564045 |

```json
{
  "userIds": ["107427060", "107347491", "107343529", "107488084"],
  "roomIds": ["53334066", "38458582", "82548523", "55564045"]
}
```

**注意**

- 含「线上环境」时须走 `online/online_execute.py`（Admin / MOA / Tunnel），勿调用测试环境 Admin。
- 跨房 PK 随机匹配前须确认双方 **App 已登录且在自己的房间内**（`onlineStatus=1` + Tunnel 近 5 分钟 heartbeat `roomId` = 自己的 roomId）。
- 线上 MOA / 造数脚本参数与测试环境不同，勿直接套用 §4.4 的测试手机号。
...
| 2026-08-11 | §4.6：线上 MENA 四账号 userIds / roomIds 数组（107427060 等） |
