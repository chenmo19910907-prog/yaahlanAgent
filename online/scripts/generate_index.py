...
def _render(registry: dict[str, Any]) -> str:
    ...
    lines.append("# Aegis 账号密码写在 Admin/.env.local（AEGIS_USERNAME / AEGIS_PASSWORD，与测试环境共用）")
    lines.append("```")
    lines.append("")
    ...
    lines.append("")
    lines.append("**Admin / MOA 自动登录**：与测试环境相同，通过 Aegis SSO 鉴权；401 或 ec=401 时自动刷新并持久化。")
    lines.append("- Admin 线上：`ADMIN_ONLINE_SSO_TOKEN` / `ADMIN_ONLINE_YAAHLAN_JWT` → `online/.env.local`")
    lines.append("- MOA 线上：`MOA_ONLINE_COOKIE` / `TUNNEL_ONLINE_COOKIE` → `online/.env.local`")
    lines.append("- 凭证：`Admin/.env.local` 的 `AEGIS_USERNAME` / `AEGIS_PASSWORD`（首次可 `python3 Admin/admin/aegis_sso.py yaahlan_online`）")
    lines.append("")
    lines.append("### 子命令速查")
    lines.append("")
...
def main() -> int:
    ...
    _sync_platform_catalog(str(online_dir().parent))
    return 0
