...
| 2026-09-01 | `/yaahlan/room/pk/openPkMode` | `/service/room/external/room-pk-api` | `openPkMode` | body：`userId`（房主/管理员）、`roomId`、`mode=3`（乱斗）、`seatCount`（8/10/12/15/20）、`minute`（10/30/60）；Tunnel `100486375` `_id=IA3IW6ABR2LVRzbvplns`（2026-09-01 15:04）；MOA 模板 `乱斗PK-创建.json` 实测 ec=200 |
| 2026-09-01 | `/yaahlan/room/pk/openPkMode` | `/service/room/external/room-pk-api` | `openPkMode` | body：`userId`、`roomId`、`mode=1`（团战）、`seatCount`（8/10/12/14 对应 4v4/5v5/6v6/7v7）、`minute`（5/15/30）；MOA 模板 `团战PK-创建.json` 实测 roomId=36043470 ec=200 |
| 2026-09-01 | `/yaahlan/room/pk/openPkMode` | `/service/room/external/room-pk-api` | `openPkMode` | body：`userId`、`roomId`、`mode=2`（1V1）、`seatCount=2`、`minute`（5/15/30）；MOA 模板 `1V1PK-创建.json` 实测 roomId=36043470 ec=200 |
| 2026-09-01 | `/yaahlan/room/pk/startPk` | `/service/room/external/room-pk-api` | `startPk` | body：`userId`（房主/管理员/主持）、`roomId`；需先 openPkMode 且满足上麦人数；MOA 模板 `房间PK-开始PK.json` 实测 roomId=36043470 ec=200 |
| 2026-09-01 | `/yaahlan/room/pk/closePkMode` | `/service/room/external/room-pk-api` | `closePkMode` | body：`userId`（房主/管理员）、`roomId`；MOA 模板 `房间PK-结束PK.json` 实测 roomId=36043470 ec=200 |
...
| 2026-09-07 | `/yaahlan/activity/familyMonsterDetail` | `/service/vas/internal/family-monster` | `getMonsterDetail` | body：`familyId`、`userId`；**当前血量直接读 remainDamage**（勿读 hp、勿用 maxHp-hp 推算；`hp`=已造成伤害）；另有 `level`/`maxHp`；活动页 HTTP 还含奖池等；Tunnel `100292275` familyId=105186 实测 remainDamage=29802 hp=198 maxHp=30000 |
| 2026-09-07 | `/yaahlan/activity/treasureHunter/prizeDraw` | `/service/vas/external/treasure-hunter-stage` | `prizeDraw` | body：`userId`/`uid`、`cardId`（`{wheelId}_{1001..1036}`，如 14_1002=第14轮第2格）；须可用探索次数；Tunnel `100070932` cardId=14_1002 实测足球券50 |
